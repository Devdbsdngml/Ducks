...
- To do
  ...
  - [x] HomeFragment Week 생리기간 가임기가 아닐 때 오늘 날짜 배경 처리 ?
  ...
  - [ ] ----------08. 05.---------- 수정 사항 끝-------------
    ...
    - [x] 취소하면 0으로 보내야됨
    - [x] chartFragment '로그인 없이 둘러보기'시 볼 수 없는 materialDialog 추가
    - [x] 푸시알림 받고 앱 들어오면 먹었어요 안 먹었어요 materialDialog 추가
    - [x] 통계 더보기 Activity 추가
    - [x] 통계 모든 페이지 데이터 연결
    - [ ] 통계 기간평균 상세는 어디에 ? (getMonthlyStatistics)
    - [x] 기초정보 페이지 EditText newLinew Block
