...
- To do
  ...
  - [ ] HomeFragment 기록하기 Btn -> CalendarFragment로 selectedDate 넘겨줘?
  ...
  - [x] HomeFragment subTitle autoLine(설정 했는데 안 먹힘;;)
    ...
    - [ ] CalendarFragment 년/월 옆에 화살표 삭제
  
    - [ ] Home이나 Calendar에 오늘 날짜 빨간 동그라미로 수정
  
    - [ ] CalendarFragment 오늘 날짜 다음 날짜들 백그라운드 alpha 설정
  
    - [ ] CalendarFragment 월 <-> 월 간격 ++
    ...
    - [x] SettingFragment
      ...
      - [x] 이메일문의 딥링크 설정
      - [ ] 로그아웃 -> 백스택에서 지워야 함
      - [ ] swithcompat livedata observe 설정 -> 저장된 값 true | false check!!
